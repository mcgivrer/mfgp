...
/**
 * MyFirstProject is a comprehensive Java-based game App application framework that provides
 ...
 *
 * @since 2025
 * @author McGivrer<mcgivrer@email.com>
 * @version 1.0.0
 */
public class App implements KeyListener{

/**
 * Enumeration representing the different operational modes of the App.
 *
 * <p>
 * These modes control the behavior and features available during App
 * execution:
 * <ul>
...
public enum AppMode {
    ...
    /**
     * Production mode.
     * Used for running the App in a production environment.
     */
    PRODUCTION,
}
    ...
    public void keyReleased(KeyEvent ke) {
        ...
        switch (ke.getKeyCode()) {
            case KeyEvent.VK_ESCAPE, KeyEvent.VK_Q -> {
                exit = true;
                debug(App.class, 1, "Exit state changed to %b", exit);
            }
            case KeyEvent.VK_PAUSE, KeyEvent.VK_P -> {
                pause = !pause;
                debug(App.class, 1, "Pause state changed to %b", pause);
            }
            case KeyEvent.VK_D -> {
                if (ke.isControlDown()) {
                    debug = (debug + 1) % 5;
                    info(App.class, "Debug level changed to %d", debug);
                }
            }
            default -> {
            }
